...
# Functions that are used for describe_data.R and modeling.R 
...
getting_accuracy <- function(dataset, seed = NULL,
  ...
  for (i in seq_along(valid_mtry)) {
    ...
    for (fold in folds) {
      ...
      cv_mtry <- c(cv_mtry, accuracy) 
      # get the current accuracy and the ones before
...
predicting_undecided <- function(dataset,
  ...
  # getting weighted results of decided
  results_decided <- dataset %>%
         group_by(Ontic) %>% 
         subset(ontic_bin == 0) %>%
  ...
  pred_relative$predictions <- round((results_decided$Freq * 
...
get_dempster_bounds <- function(dataset, party = c("SPD", "CDU.CSU", "FDP",
  ...
  if(type == "coalitions"){
    ...
    for(i in 1:nrow(dataset_lower)) {
      ...
      if(dataset_lower[["ontic_bin"]][i] == 0 && 
         sum(dataset_lower[coalition][i, ]) == 1){
        sum_decided_lower <- sum_decided_lower + dataset_lower[["Gewicht"]][i]
      }
    }
    # upper bound
    for(i in 1:nrow(dataset_upper)) { 
      ...
      if(dataset_upper[["ontic_bin"]][i] == 0 && 
         sum(dataset_upper[coalition][i, ]) == 1){
        sum_decided_upper <- sum_decided_upper + dataset_upper[["Gewicht"]][i]
      }
    }
...
# C) Plotting decided and undecided people with and without weights
...
plotting_undecided_weights <- function(dataset){
  dataset %>%
    group_by(Ontic) %>% 
    subset(ontic_bin == 1) %>%
    summarise(count = sum(Gewicht)) %>%  # using Gewicht instead of count 
...
# D) Plotting Sonntagsfrage including weights
