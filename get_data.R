...
no_others <- function(dataset, variable = "Ontic"){
  ...
  dataset[, variable] <- str_remove_all(dataset[, variable], 
                                        '-.-Eine andere Partei')
...
data_all_missings <- data_all %>%
  show_na 

# Scaling of the weight to get again the number of observations 
# as the sum of the weight
wave2$Gewicht <-
  wave2$Gewicht + ((nrow(wave2) - sum(wave2$Gewicht)) / sum(wave2$Gewicht))
wave3$Gewicht <-
  wave3$Gewicht + ((nrow(wave3) - sum(wave3$Gewicht)) / sum(wave3$Gewicht))
...
write.csv(data_all_missings, './Data/data_all_missings.csv')
