...
accuracy_wave2_civey <- getting_accuracy(dataset = wave2, 
                                         seed = 11561523, 
                                         weight_type = "civey")

# Accuracy comparison - no weights, civey weights and simulation weights
accuracy_no_weights_wave2 <- getting_accuracy(dataset = wave2, seed = 11561523,
                                              weight_type = "none")[["accuracy"]]
accuracy_civey_wave2 <- getting_accuracy(dataset = wave2, seed = 11561523,
                                         weight_type = "civey")[["accuracy"]]
accuracy_simulation_wave2 <- getting_accuracy(dataset = wave2, seed = 11561523,
                                              weight_type = "simulation")[["mean_accuracy_sim"]]

comparison_accuracy_wave2 <- data.frame(`No weights` = round(accuracy_no_weights, 3),
                                        `Civey weights` = round(accuracy_civey,3 ),
                                        `Simulated weights` = round(accuracy_simulation, 3))
...
# Accuracy for Cross validation and best mtry
accuracy_wave3_civey <- getting_accuracy(dataset = wave3, 
                                         seed = 11561523, 
                                         weight_type = "civey")

# Accuracy comparison - no weights, civey weights and simulation weights
accuracy_no_weights_wave3 <- getting_accuracy(dataset = wave3, seed = 11561523,
                                              weight_type = "none")[["accuracy"]]
accuracy_civey_wave3 <- getting_accuracy(dataset = wave3, seed = 11561523,
                                         weight_type = "civey")[["accuracy"]]
accuracy_simulation_wave3 <- getting_accuracy(dataset = wave3,seed = 11561523,
                                              weight_type = "simulation")[["mean_accuracy_sim"]]

comparison_accuracy_wave3 <- data.frame(`No weights` = round(accuracy_no_weights_wave3, 3),
                                        `Civey weights` = round(accuracy_civey_wave3, 3),
                                        `Simulated weights` = round(comparison_accuracy_wave3, 3))
...
sim_weight_wave2 <- plotting_simulation_example(wave2, "Simuliertes Gewicht")
...
comparison_res_dempster <- res_dempster_plot_wave2 + 
  res_dempster_plot_wave2 + plot_annotation(
    title = 'Dempster Bounds - 80/20 Annahme', 
    theme = theme(plot.title = element_text(size = 16)))
...
possible_coalitions <- list(c("CDU.CSU", "FDP"), c("SPD", "Grüne"), 
                            c("SPD", "Grüne", "Linke"), c("CDU.CSU", "FDP", "AfD"), 
...
comparison_coalitions <- dempster_coalitions_plot_wave2 + 
  dempster_coalitions_plot_wave3 + plot_annotation(
    title = 'Dempster Bounds - Partitionierungen', 
    theme = theme(plot.title = element_text(size = 16)))
...
# Different title for the prediciton results of the random forest
# i. Wave 2
rf_predictions_wave2 <- predicting_undecided(dataset = wave2, 
                                             weight_type = "civey",
                                             title = "Homogenitätsannahme", 
...
annotate_figure(comparison_total, 
                top = text_grob("Vergleich der Prognosen", size = 16))
...
# i. Wave 2
# Results of decided people without weights
decided_wave2_no_weights <- plotting_decided(dataset = wave2)

# ii. Wave 3
# Results of decided people without weights
decided_wave3_no_weights <- plotting_decided(dataset = wave3)

# i. Wave 2
# Results of Sonntagsfrage without weights
forced_decision_wave2_no_weights <- plotting_forced_decision_no_weights(wave2)

# ii. Wave 3
# Results of Sonntagsfrage without weights
forced_decision_wave3_no_weights <- plotting_forced_decision_no_weights(wave3)

# i. Wave 2
# Results of random forest without weights
rf_predictions_wave2_no_weights <- predicting_undecided(dataset = wave2, 
                                                        weight_type = "none",
                                                        title = "Homogenitätsannahme", 
                                                        best_mtry = 3)
...
# ii. Wave 3
# Results of random forest without weights
rf_predictions_wave3_no_weights <- predicting_undecided(dataset = wave3, 
                                                        weight_type = "none",
                                                        title = "Homogenitätsannahme", 
                                                        best_mtry = 2)

# Get all plots without weights next to each other
comparison_total_no_weights <- ggarrange(decided_wave2_no_weights, 
                                         decided_wave3_no_weights,
                                         rf_predictions_wave2_no_weights, 
                                         rf_predictions_wave3_no_weights,
                                         forced_decision_wave2_no_weights, 
                                         forced_decision_wave3_no_weights
                                         + rremove("x.text"), common.legend = TRUE,
                                         ncol = 2, nrow = 3)

annotate_figure(comparison_total_no_weights, 
                top = text_grob("Vergleich der Prognosen ohne Gewichtung",
                                size = 16))


ggsave("./Plots/comparison_total_no_weights.png",
       width = 11,
       height = 6,
       dpi = 1000)
